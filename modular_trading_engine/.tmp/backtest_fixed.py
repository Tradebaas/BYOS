...
class FixedSimulator(BacktestSimulator):
    def _evaluate_active_position(self, candle: Candle) -> None:
        ...
        # FIXED TP/SL: 12 points
        FIXED_PTS = 12.0
        
        if intent.is_bullish:
            tp = intent.entry_price + FIXED_PTS
            sl = intent.entry_price - FIXED_PTS
        else:
            tp = intent.entry_price - FIXED_PTS
            sl = intent.entry_price + FIXED_PTS
    ...
    def _close_position_custom(self, candle: Candle, pos: ActivePosition, exit_price: float, is_win: bool) -> None:
        ...
        record = TradeRecord(
            ...
            stop_loss=intent.entry_price - 12.0 if intent.is_bullish else intent.entry_price + 12.0,
            take_profit=exit_price if is_win else intent.take_profit,
            mfe=pos.mfe,
            mae=pos.mae,
            win=(pnl > 0),
            pnl_points=pnl
...
def run_fixed_backtest():
    ...
    logging.info(f"Running Fixed 12/12 Backtest for {strategy_dir}")
    ...
    logging.info("\n==============================================")
    logging.info("📈 SIMULATIE RAPPORT - FIXED 12 TP / 12 SL")
    logging.info("==============================================")
