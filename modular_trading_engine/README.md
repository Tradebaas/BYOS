# Modular Trading Engine (DTD Golden Setup)

Welkom bij de **Modular Trading Engine**. Dit is een volledig geautomatiseerde, head-less Python trading bot ontworpen volgens de stikte **GSD (Get Shit Done)** architectuur. De engine is primair ontworpen voor live executie op prop-firm accounts via de TopstepX (ProjectX) API, met een focus op snelheid, robuustheid, en strikt risicobeheer via geautomatiseerde trailing stop-losses.

Deze documentatie dient als jouw **A-Z Handleiding**. Alles wat je nodig hebt om een nieuwe machine in te richten, een live-account te koppelen en de bot of backtester te starten staat hier beschreven.

---

## 🏗️ De Architectuur (De 4 Lagen)
...
Kopieer `.env.example` naar `.env` in de root directory:
```bash
cp .env.example .env
```
...
1. Draai het configuratiescript om je actieve accounts en bijbehorende ID's op te halen:
   ```bash
...
```bash
# Zorg dat je in de virtual environment zit, en draai:
...
Een overzicht van het landschap voor als je zelf zaken gaat ontwikkelen:
